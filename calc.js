...
//adding ** sign to screen can also be done afyter performing operation
document.getElementById("xraisey").addEventListener("click", function() {
  opera = 0;
  setvalue('**');
});
...
document.getElementById("tenraisex").addEventListener("click", function() {
  let x = getvalue();
  try {
...
document.getElementById("plusorminus").addEventListener("click", function() {
  let x = getvalue();
  try {
  	if(screen.value <= 0)
    	screen.value = Math.abs(eval(x));
